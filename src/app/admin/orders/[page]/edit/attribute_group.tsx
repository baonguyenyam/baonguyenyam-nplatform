...
export default function OrderAttributeGroup(props: any) {
	...
	const availableAttsForActiveGroup = useMemo(() => {
		...
		const selectedIds = Array.isArray(activeGroup.attributes)
			? activeGroup.attributes.map((attr) => attr.id)
			: [];
	...
	const hasChanges = useMemo(() => {
		...
		const currentString = Array.isArray(groupSelected)
			? JSON.stringify(groupSelected)
			: "[]";
		const savedString = Array.isArray(savedGroupData)
			? JSON.stringify(savedGroupData)
			: "[]";
	...
	const handleSelectAttributeForGroup = (attributeDefinition: any) => {
		...
		const nextGroupSelected = produce(groupSelected, (draft) => {
			...
			if (
				group &&
				!group.attributes.some((attr) => attr.id === newAttributeInstance.id)
			) {
				group.attributes.push(newAttributeInstance);
			} else if (group) {
				toast.info(
					`Attribute "${newAttributeInstance.title}" already added to this group.`,
				);
	...
	const handleDeleteAttributeInstance = (attributeId: string) => {
		...
		if (
			!confirm(
				`Are you sure you want to remove the entire "${activeGroup?.attributes.find((a) => a.id === attributeId)?.title}" attribute section from this group?`,
			)
		)
			return;
	...
	const handleDeleteCheckboxSingleValue = (
		...
		handleUpdateCheckboxValue(
			...
			valueToRemove,
			false,
		); // Use the existing logic to remove
	};
	...
	const handleDeleteGroup = (groupId: string) => {
		...
		if (
			!confirm(
				`Are you sure you want to delete the group "${groupToDelete.title}"?`,
			)
		)
			return;
		...
		if (activeGroupId === groupId) {
			...
			nextActiveGroupId =
				nextGroupSelected.length > 0 ? nextGroupSelected[0].id : null;
						...
						<div className="group flex items-center">
								...
								{groupSelected.map((group) => (
									<TabsTrigger
										...
										className={cn(
											...
											{
												"border-black border-r-gray-200":
							...
							{!permission && (
								<Dialog
									...
									onOpenChange={(isOpen) =>
										setOpen([isOpen ? "create-group" : "", null])
									}
								>
					...
					{groupSelected.map((group) => (
						...
						>
							...
							<div className="flex flex-col space-y-4">
											...
											>
												...
												<tbody>
													{attributeInstance.children?.map((row, rowIndex) => {
														...
														const attributeDefinition = atts.find(
															(att: any) => att.id === attributeInstance.id,
														);
														const colCount =
															attributeDefinition?.children?.length ?? 1;
														return (
															<tr
															...
															>
																...
																{row.map((field, fieldIndex) => {
																	...
																	const fieldType =
																		fieldDefinition?.type ?? "text"; // Default to text

																	const getFParentInstanceId =
																		attributeInstance.id; // Use attribute instance ID
																	const getFieldId =
																		fieldDefinition?.id ?? field.id; // Use field definition ID if available
																	...
																	const orderPermissionItem =
																		orderPermission
																			?.find(
																				(item: any) =>
																					item.id ===
																					Number(getFParentInstanceId),
																			)
																			?.children?.find(
																				(item: any) =>
																					item.id === Number(getFieldId),
																			)
																			?.permission.find(
																				(item: any) => item.key === tab,
																			)?.checked ?? true;

																	return (
																		<Fragment
																			key={`${group.id}-${attributeInstance.id}-row-${rowIndex}-field-${field.id}`}
																		>
																			<td
															...
															<tr className="text-center text-sm text-gray-500 dark:text-gray-400">
																<td
																	...
																	colSpan={
																		!permission
																			? Number(
																					longestRowForHeader?.length ?? 0,
																				) + 2
																			: (longestRowForHeader?.length ?? 0)
																	}
																>
																	No rows added yet for "
																	{attributeInstance.title}". Click{" "}
																	<PlusCircle className="w-4 h-4 inline-block mx-1" />{" "}
																	above to add one.
																</td>
															</tr>
														)}
									...
									>
										...
										<DialogContent className="w-full sm:max-w-[450px] dark:bg-gray-800 dark:border-gray-700">
											...
											<div className="flex flex-col max-h-[400px] overflow-y-auto">
												{availableAttsForActiveGroup.map(
													(item: any, index: number) => (
														...
														>
															...
															<Button
																...
																className="text-xs"
																onClick={() =>
																	handleSelectAttributeForGroup(item)
																}
															>
																Add
															</Button>
								...
								{group.attributes?.length === 0 && !permission && (
					...
					>
						...
						<DialogContent className="w-full sm:max-w-[450px] dark:bg-gray-800 dark:border-gray-700">
							...
							<Button
								...
								onClick={() =>
									handleAddGroup(
										(document.querySelector("#group-name") as HTMLInputElement)
											?.value,
									)
								}
							>
