...
export default function AttributeRoot(props: any) {
	const { data } = props;
	...
	const findFieldDefinition = (attributeId: string, fieldId: string) => {
		...
		return attributeDefinition?.children?.find(
			(fieldDef: any) => fieldDef.id === fieldId,
		);
	};
		...
		<div className="block w-full">
				...
				<div className="flex flex-col space-y-6">
					...
					{groupSelected.map((group) => (
						<div key={group.id} className="group-section mb-10">
							<h2 className="text-xl font-semibold mb-3 border-b pb-2 dark:border-gray-700">
								{group.title}
							</h2>
							{group.attributes && group.attributes.length > 0 ? (
								<div className="flex flex-col space-y-4">
									...
									{group.attributes.map((attributeInstance) => {
										...
										return (
											...
											>
												<h3 className="text-base font-medium mb-2 pl-1">
												...
												attributeInstance.children.length > 0 ? (
													...
													>
														...
														<tbody>
															...
															{attributeInstance.children.map(
																(row, rowIndex) => (
																	<tr
																		key={rowIndex}
																	...
																	>
																		...
																		{row.map((field) => {
																			...
																			const fieldType =
																				fieldDefinition?.type ?? "text"; // Default to text if definition not found
													...
													<p className="text-sm text-gray-500 dark:text-gray-400 italic pl-1">
														No data recorded for this attribute.
													</p>
												)}
											</div>
										);
								...
								<p className="text-sm text-gray-500 dark:text-gray-400 italic">
									No attributes added to this group.
								</p>
							)}
						</div>
					))}
			...
			{atts?.length === 0 && groupSelected.length > 0 && (
				<div className="mt-4 text-sm text-orange-600 dark:text-orange-400">
					Warning: Attribute definitions seem to be missing. Display might be
					incomplete.
				</div>
			)}
		</div>
