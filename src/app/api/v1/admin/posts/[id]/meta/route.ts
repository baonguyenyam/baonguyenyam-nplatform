...
export async function GET(req: Request) {
	...
	const db = await models.PostMeta.getPostMetaByPostID(Number(params));
...
export async function POST(req: Request) {
	...
	const params = req.url.split("/").slice(-2, -1)[0];
	// Add the postId to the body
	body.postId = Number(params);
	const db = await models.PostMeta.createPostMeta(body);
	if (session && db) {
		return new Response(
			JSON.stringify({
