...
export default function AppStatus(props: any) {
	...
	const border = status?.borderClassName
	...
	return (
		<>
			<div
				className={cn(
					size === "small"
						? "inline-flex flex-row items-center px-2 py-[0.15em] rounded-full border space-x-1"
			...
			>
				{IconComponent && (
					<IconComponent
						className={size === "small" ? "w-3 h-3 mr-1" : "w-4 h-4 mr-1"}
					/>
				)}
