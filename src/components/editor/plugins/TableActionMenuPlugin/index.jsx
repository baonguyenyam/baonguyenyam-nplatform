...
function currentCellBorderColor(editor) {
...
function TableActionMenu({
  ...
  const [borderColor, setBorderColor] = useState(() => currentCellBorderColor(editor) || "");
    ...
    return editor.registerMutationListener(
      ...
      (nodeMutations) => {
        ...
        if (nodeUpdated) {
          ...
          setBorderColor(currentCellBorderColor(editor) || "");
  ...
  const rotateTableCell = (isLeft = false) => {
    editor.update(() => {
      const selection = $getSelection();
      if ($isRangeSelection(selection) || $isTableSelection(selection)) {
        if ($isTableSelection(selection)) {
          ...
          nodes.map((node) => {
            if ($isTableCellNode(node)) {
              return isLeft ? node.rotateLeft() : node.rotateRight();
            }
            return node;
          });
        } else {
          const [cell] = $getNodeTriplet(selection.anchor);
          if ($isTableCellNode(cell)) {
            isLeft ? cell.rotateLeft() : cell.rotateRight();
          }
        }
      }
    });
  ...
  const handleCellBorderColor = useCallback(
    (value) => {
      editor.update(() => {
        const tableNode = $getTableNodeFromLexicalNodeOrThrow(tableCellNode);

        if (tableNode) {
          const nodes = tableNode.getChildren();
  ...
  return createPortal(
    ...
    >
      ...
      <button
        ...
        className="item"
        onClick={() =>
          showColorPickerModal("Cell border color", () => (
            <ColorPicker color={borderColor} onChange={handleCellBorderColor} />
          ))
        }
        data-test-id="table-border-color"
      >
      ...
      <button
        type="button"
        className="item"
        onClick={() => rotateTableCell(true)}
        data-test-id="table-rotate-cell-left"
      >
        <span className="text">Rotate 90 degrees to the left</span>
      </button>
      <button
        type="button"
        className="item"
        onClick={() => rotateTableCell(false)}
        data-test-id="table-rotate-cell-right"
      >
        <span className="text">Rotate 90 degrees to the right</span>
      </button>
      <hr />
      <button
        type="button"
