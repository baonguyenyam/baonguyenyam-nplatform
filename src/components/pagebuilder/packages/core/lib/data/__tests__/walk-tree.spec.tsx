import { createAppStore, defaultAppState as _defaultAppState } from "../../../store";
...
const expectIndexed = (state: PrivateAppState, item: ComponentData | undefined, path: string[], index: number) => {
	if (!item) return;

	const zoneCompound = path[path.length - 1];

	expect(state.indexes.zones[zoneCompound]?.contentIds[index]).toEqual(item.props.id);
...
describe("walk-tree", () => {
	...
	it("should generate the correct index for a given payload", () => {
		...
		expectIndexed(state, state.data.zones?.["my-component:zone"][0], [rootDroppableId, "my-component:zone"], 0);

		expectIndexed(state, state.data.zones?.["other-component:zone"][0], [rootDroppableId, "my-component:zone", "other-component:zone"], 0);

		expectIndexed(state, state.data.zones?.["other-component:zone"][0].props.slotA[0], [rootDroppableId, "my-component:zone", "other-component:zone", "another-id:slotA"], 0);

		expectIndexed(state, state.data.zones?.["other-component:zone"][0].props.slotB[0], [rootDroppableId, "my-component:zone", "other-component:zone", "another-id:slotB"], 0);
