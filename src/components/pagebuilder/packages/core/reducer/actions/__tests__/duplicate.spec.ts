...
import { defaultData, defaultState, dzZoneCompound, expectIndexed, testSetup } from "../__helpers__";
	...
	describe("duplicate action", () => {
		describe("with DropZones", () => {
			...
			it("should recursively duplicate related items and zones", () => {
				...
				expectIndexed(newState, zone2[0], [rootDroppableId, zone2ZoneCompound], 0);
				...
				expectIndexed(newState, zone3[0], [rootDroppableId, zone2ZoneCompound, zone3ZoneCompound], 0);
			});
		...
		describe("with slots", () => {
			...
			it("should duplicate within a slot, within a slot", () => {
				...
				const content = (newState.data.root.props?.slot ?? [])[0].props.slot as Content;
				expect(content).toHaveLength(2);
			...
			it("should duplicate within a slot, within a DropZone", () => {
				...
				const content = newState.data.zones?.[dzZoneCompound][0].props.slot || [];

				expect(content).toHaveLength(2);
				expect(content[1].props.id).not.toBe("second");
				expect(content[1].props.prop).toBe("Some example data");
				expectIndexed(newState, content[1], [rootDroppableId, dzZoneCompound, "first:slot"], 1);
			});
