...
import { defaultState, dzZoneCompound, expectIndexed, testSetup, UserData } from "../__helpers__";
	...
	describe("insert action", () => {
		...
		describe("with slots", () => {
			...
			it("should insert into a slot within a slot", () => {
				...
				expect(item).toHaveProperty("type", "Comp");
				expect(item?.props).toHaveProperty("prop", "example");
				expectIndexed(state, item, ["root:slot", `${state.data.root.props?.slot[0].props?.id}:slot`], 0);
			});
