...
import { defaultState, dzZoneCompound, expectIndexed, testSetup } from "../__helpers__";
	...
	describe("move action", () => {
		describe("with DropZones", () => {
			...
			it("should move items between zones", () => {
				...
				expect(newState.data.content[0].props.id).toBe("2");
				expectIndexed(newState, newState.data.zones?.[dzZoneCompound][0], [rootDroppableId, dzZoneCompound], 0);
				expectIndexed(newState, newState.data.content[0], [rootDroppableId], 0);
			});
		});
		...
		describe("with slots", () => {
			it("should reorder within a slot", () => {
				...
				expectIndexed(newState, newState.data.root.props?.slot[0], ["root:slot"], 0);
				expectIndexed(newState, newState.data.root.props?.slot[1], ["root:slot"], 1);
			});
			...
			it("should move items from a deep slot to a zone", () => {
				...
				expectIndexed(newState, item3, [rootDroppableId, dzZoneCompound, "2:slot"], 0);
			});
		});
	});
});
