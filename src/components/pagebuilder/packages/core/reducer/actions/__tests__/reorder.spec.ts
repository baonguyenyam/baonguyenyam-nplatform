	...
	describe("reorder action", () => {
		describe("with DropZones", () => {
			...
			it("should reorder within a different zone", () => {
				...
				expectIndexed(newState, newState.data.content[0], [rootDroppableId, dzZoneCompound], 0);
				expectIndexed(newState, newState.data.content[1], [rootDroppableId, dzZoneCompound], 1);
			});
		});
