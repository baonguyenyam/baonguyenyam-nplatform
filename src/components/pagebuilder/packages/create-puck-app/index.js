  ...
  `,
	...
	.action(async (_appName, options) => {
		...
		if (fs.existsSync(appPath)) {
			console.error(`A directory called ${appName} already exists. Please use a different name or delete this directory.`);
			return;
		}
		...
		for (const templateFile of templateFiles) {
			...
			const targetPath = filePath.replace(templatePath, appPath).replace(".hbs", "").replace("gitignore", ".gitignore"); // Rename gitignore back to .gitignore (.gitignore) gets ignored by npm during publish
