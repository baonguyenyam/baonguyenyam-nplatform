...
export function createFieldContentful<T extends Entry = Entry>(
	...
	const { space, accessToken, titleField = "title", filterFields, initialFilters } = options;

	if (!options.client) {
		if (!space || !accessToken) {
			throw new Error('field-contentful: Must either specify "client", or "space" and "accessToken"');
		}
	}

	const client = options.client || createClient({ space: space!, accessToken: accessToken! });
	...
	const field: ExternalField<T> = {
		...
		getItemSummary: (item) => item.fields[titleField as keyof typeof item.fields],
		filterFields,
		initialFilters,
	};
