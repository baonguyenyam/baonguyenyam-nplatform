		...
		if (state?.pageSize === undefined) {
			...
			if (res.success === "success") {
				dispatch(
					SET_APP_STATE({
						pageSize:
							res.data?.find((item: any) => item.key === "page")?.value ?? 10,
						title:
							res.data?.find((item: any) => item.key === "title")?.value ??
							appState.appName,
						description:
							res.data?.find((item: any) => item.key === "description")
						...
						bill_company_phone:
							res.data?.find((item: any) => item.key === "bill_company_phone")
								?.value ?? "",
						order_permission: res.data?.find(
							(item: any) => item.key === "order_permission",
						)?.value
							? JSON.parse(
									res.data?.find((item: any) => item.key === "order_permission")
										?.value ?? "[]",
								)
							: [],
					}),
